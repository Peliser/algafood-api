...
import com.algaworks.algafood.api.v1.controller.ValidacaoException;
...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<?> handleEntidadeNaoEncontrada(AccessDeniedException ex, WebRequest request) {
        ...
        Problem problem = toProblemBuilder(status, problemType, detail).userMessage(detail)
    ...
    protected ResponseEntity<Object> handleExceptionInternal(Exception ex, Object body, HttpHeaders headers,
        ...
        if (body == null) {
            body = Problem.builder().title(status.getReasonPhrase()).status(status.value())
                    .userMessage(MSG_ERRO_GENERICO_USUARIO_FINAL).timestamp(OffsetDateTime.now()).build();
    ...
    private ResponseEntity<Object> handleValidationInternal(Exception ex, BindingResult bindingResult,
        ...
        List<Problem.Error> problemObjects = bindingResult.getAllErrors().stream().map(objectError -> {
            ...
            return Problem.Error.builder().name(name).userMessage(message).build();
        }).collect(Collectors.toList());
        Problem problem = toProblemBuilder(status, ProblemType.DADOS_INVALIDOS, detail).userMessage(detail)
                .errors(problemObjects).build();
        return handleExceptionInternal(ex, problem, headers, status, request);
    }
