...
public class SpringFoxConfig implements WebMvcConfigurer {
    ...
    /**
     * Comment the @Bean to turn off the documentation of the version.
     */
    @Bean
...
//                        .parameterType("query").modelRef(new ModelRef("string")).build()))
    ...
    private List<ResponseMessage> globalGetResponseMessages() {
        return Arrays.asList(
                new ResponseMessageBuilder().code(HttpStatus.INTERNAL_SERVER_ERROR.value())
                        .responseModel(new ModelRef("Problema")).message("Erro interno do servidor").build(),
                new ResponseMessageBuilder().code(HttpStatus.NOT_ACCEPTABLE.value()).message("Representacão não aceita")
                        .build());
    }
    ...
    private List<ResponseMessage> globalPostPutResponseMessages() {
        return Arrays.asList(new ResponseMessageBuilder().code(HttpStatus.BAD_REQUEST.value())
                ...
                new ResponseMessageBuilder().code(HttpStatus.INTERNAL_SERVER_ERROR.value())
                        .responseModel(new ModelRef("Problema")).message("Erro interno no servidor").build(),
    ...
    private ApiInfo apiInfoV1() {
        return new ApiInfoBuilder().title("AlgaFood API (depreciada)").description(
                "API para clientes e restaurantes.<br><strong>Essa versão da API está depreciada. Use a versão mais atual.")
                .version("1").contact(new Contact("AlgaWorks", "https://www.algaworks.com", "contato@algaworks.com"))
                .build();
    }

    private ApiInfo apiInfoV2() {
        return new ApiInfoBuilder().title("AlgaFood API").description("Api para clientes e restaurantes").version("2")
